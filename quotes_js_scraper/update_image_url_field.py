# Script to adding images from local to firestore
# import os
...
# def upload_images_and_update_firestore_for_city(city_id):
#     # Initialize Firebase
#     db, bucket = initialize_firebase()
    
#     # Path to the 'categories search' folder
#     base_folder_path = r"C:\dev\python_runs\scrapy_selenium\quotes-js-project\Categories Search"

#     city_folder_path = os.path.join(base_folder_path, city_id)
#     if not os.path.isdir(city_folder_path):
#         print(f"City folder for city_id {city_id} does not exist at {city_folder_path}")
#         return  # Exit the function if city folder doesn't exist

#     print(f"Processing city ID: {city_id}")

#     # Loop through each playlist folder (title) within the city folder
#     for playlist_title in os.listdir(city_folder_path):
#         playlist_folder_path = os.path.join(city_folder_path, playlist_title)
#         if not os.path.isdir(playlist_folder_path):
#             continue  # Skip if not a directory
#         print(f"  Processing playlist: {playlist_title}")
        
#         # Find image files in the playlist folder
#         image_files = [
#             f for f in os.listdir(playlist_folder_path)
#             if os.path.isfile(os.path.join(playlist_folder_path, f))
#         ]
        
#         if not image_files:
#             print(f"    No image files found in {playlist_folder_path}")
#             continue
        
#         # Assuming one image per playlist folder
#         image_file = image_files[0]
#         local_image_path = os.path.join(playlist_folder_path, image_file)
        
#         # Upload the image to Firebase Storage
#         storage_path = f"playlists/{city_id}/{playlist_title}/{image_file}"
#         try:
...
#             print(f"    Uploaded image to {storage_path}, URL: {image_url}")
#         except Exception as e:
#             print(f"    Error uploading {local_image_path}: {e}")
#             continue  # Skip to next playlist
        
#         # Find the corresponding document in 'playlists' collection
#         try:
#             # Query the 'playlists' collection for matching city_id and title
#             playlist_query = db.collection('playlists').where('city_id', '==', city_id).where('title', '==', playlist_title).limit(1)
#             playlist_docs = playlist_query.get()
            
#             if not playlist_docs:
#                 print(f"    No matching playlist document found for title: {playlist_title} and city_id: {city_id}")
#                 continue
            
#             playlist_doc = playlist_docs[0]
#             playlist_doc_ref = playlist_doc.reference
#             # Update the 'imageUrl' field
#             playlist_doc_ref.update({'imageUrl': image_url})
#             print(f"    Updated 'imageUrl' field in document ID: {playlist_doc.id}")
#         except Exception as e:
#             print(f"    Error updating Firestore document for playlist {playlist_title}: {e}")
#             continue  # Skip to next playlist

# if __name__ == '__main__':
#     # Set the city_id you want to process
#     city_id = '57'  # City ID for the city you want to process

#     upload_images_and_update_firestore_for_city(city_id)
...
# Initialize Firestore client and get the Storage bucket
db = firestore.client()
bucket = storage.bucket()
...
def get_all_playlists(playlists_ref, max_retries=3, timeout=300):
    """
    Retrieves all documents from the given Firestore collection reference.
    Retries if a DeadlineExceeded error occurs.
    """
    for attempt in range(max_retries):
        try:
            playlists = list(playlists_ref.stream(timeout=timeout))
            return playlists
        except DeadlineExceeded as e:
            print(f"Attempt {attempt+1}: DeadlineExceeded error while streaming playlists: {e}. Retrying in 5 seconds...")
            time.sleep(5)
...
def generate_image_for_playlist(title):
    """
    Uses OpenAI's DALL·E 3 to generate an image based on the playlist title.
    Returns the image URL from OpenAI.
    """
    prompt = f"Generate a creative, high-quality image that represents a playlist titled '{title}'.Generate a realistic image"
    try:
        ...
        print(f"Error generating image for playlist '{title}': {e}")
        return None

def download_image(image_url):
    """
    Downloads the image from the provided URL and returns the image bytes.
    """
    try:
...
def upload_image_to_storage(playlist_id, image_bytes):
    ...
    try:
        ...
        print(f"Error uploading image for playlist '{playlist_id}' to Storage: {e}")
        return None

def process_playlist_images():
    """
    Processes the 'playlists' collection in Firestore:
      - For each playlist missing an 'imageUrl', generate an image using DALL·E 3,
        download it, upload it to Firebase Storage, and update the document with the public URL.
    """
    playlists_ref = db.collection('playlistsNew')
    
    # Use the helper function to fetch all playlists with extended timeout and retries.
    ...
    for playlist in playlists:
        ...
        # Skip playlists that already have an imageUrl
        if playlist_data.get('imageUrl'):
            print(f"Document ID {doc_id} already has an imageUrl. Skipping...")
            continue

        print(f"Processing playlist '{title}' (Document ID: {doc_id})")
        dalle_image_url = generate_image_for_playlist(title)
        if not dalle_image_url:
            print(f"Failed to generate image for playlist '{title}'")
            continue

        image_bytes = download_image(dalle_image_url)
        if not image_bytes:
            print(f"Failed to download image for playlist '{title}'")
            continue
        ...
        if public_url:
            try:
                ...
                print(f"Updated document ID {doc_id} with imageUrl: {public_url}")
            except Exception as e:
                print(f"Error updating document ID {doc_id}: {e}")
        else:
            print(f"Failed to upload image for playlist '{title}' (Document ID: {doc_id})")
...
# # Initialize Firestore client and get the default Storage bucket
# db = firestore.client()
# bucket = storage.bucket()
...
# # Initialize Firebase Storage client with explicit credentials and project id
# # Replace 'YOUR_PROJECT_ID_HERE' with your actual project id (usually found in your Firebase console)
# FIREBASE_PROJECT_ID = "YOUR_PROJECT_ID_HERE"
# storage_client = storage.Client(credentials=cred, project=FIREBASE_PROJECT_ID)
# # Replace with your Firebase Storage bucket name (typically "your-project-id.appspot.com")
...
# def download_image(image_url):
#     """
#     Downloads the image from the provided URL and returns image bytes.
#     """
#     try:
#         response = requests.get(image_url)
#         response.raise_for_status()
...
#     Makes the blob public and returns the public URL.
#     """
#     filename = f"playlist_images/{playlist_id}.png"
#     blob = bucket.blob(filename)
#     try:
#         blob.upload_from_string(image_bytes, content_type="image/png")
