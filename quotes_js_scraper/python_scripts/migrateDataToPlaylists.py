...
"""
# SCRIPT INDEX AND SUMMARY
# ======================
# This file contains multiple scripts for managing Firestore data migrations and operations.
# Below is an index of all scripts with a brief description of each:

# [1] Main Script - Process Parent Documents with Image URLs
#     - Active script that processes documents in the 'explore' collection
#     - Updates documents with top image URLs from subcollections
#     - Handles missing 'topAttractionImageUrl' and 'topRestaurantImageUrl' fields

# [2] Check Missing Fields (Commented Out)
#     - Scans 'explore' collection for documents missing image URL fields
#     - Prints document IDs with missing fields and summary statistics

# [3] Data Migration Script (Commented Out)
#     - Migrates data from 'searchedCategories2' to 'playlistsNew' collection
#     - Processes city documents and their playlists
#     - Assigns sequential list IDs to playlists

# [4] Document Processing Utilities (Commented Out)
#     - Contains various utility functions for document processing
#     - Includes functions for checking subcollections and copying data

# [5] Google Places Integration (Commented Out)
#     - Functions for interacting with Google Places API
#     - Handles place ID lookup and photo downloads

# [6] Firebase Storage Operations (Commented Out)
#     - Functions for uploading and managing files in Firebase Storage
#     - Handles image uploads and metadata management

# [7] Collection Management (Commented Out)
#     - Utilities for copying and managing Firestore collections
#     - Includes batch processing and error handling

# [8] Image Processing (Commented Out)
#     - Functions for processing and uploading images
#     - Handles various image formats and metadata

# [9] Data Validation (Commented Out)
#     - Scripts for validating data integrity
#     - Checks for required fields and data consistency

# [10] Document Cleanup (Commented Out)
#      - Utilities for cleaning up documents
#      - Handles field updates and data normalization

# Active script starts below:
"""

# ===== [2] CHECK MISSING FIELDS SCRIPT =====
...
#     """
#     Processes documents in the 'explore' collection and prints the IDs of those
#     documents that are missing either 'topAttractionImageUrl' or 'topRestaurantImageUrl'.
#     Also prints the total number of such documents.
#     """
#     db = firestore.client()
...
#                 missing_attraction = 'topAttractionImageUrl' not in doc_data
#                 missing_restaurant = 'topRestaurantImageUrl' not in doc_data

#                 if missing_attraction or missing_restaurant:
#                     print(f"Document missing fields: {doc.id}")
#                     missing_count += 1

#                 last_doc = doc  # Update last_doc for pagination

#         except DeadlineExceeded as e:
#             print(f"Deadline exceeded during query. Retrying after a short pause... {e}")
#             time.sleep(5)
#             continue
#         except Exception as e:
...
#     print(f"Total documents missing fields: {missing_count}")
...
# ===== [1] MAIN SCRIPT - Process Parent Documents with Image URLs =====
"""
Script to migrate data from Firestore 'explore' collection to 'playlists' collection.
For each document in the 'explore' collection, it creates a new document in the 'playlists' collection with the following fields:
- title: The document ID from the 'explore' collection.
- description: The 'g_description' field from the 'explore' document.
- category: The 'category' field from the 'explore' document.
- imageUrl: The 'topAttractionImageUrl' or 'topRestaurantImageUrl' field from the 'explore' document.
"""
import firebase_admin
from firebase_admin import credentials, firestore
import sys
...
def get_top_image_url(subcollection_ref):
    """
    Retrieves the top image URL from the second document in the subcollection.

    :param subcollection_ref: Reference to the subcollection.
    :return: The image URL from the second document or None if not found.
    """
    try:
        # Retrieve up to two documents from the subcollection
        docs = list(subcollection_ref.limit(5).stream())
        if len(docs) >= 7:
            # Select the second document
            second_doc = docs[5]
            data = second_doc.to_dict()
            g_image_urls = data.get('g_image_urls')
            if g_image_urls and isinstance(g_image_urls, list) and len(g_image_urls) > 0:
                return g_image_urls[0]  # Return the first image URL from the second document
        return None  # Return None if no second document or no image URLs found
    except Exception as e:
    ...
    while True:
        try:
            ...
            for doc in docs:
                ...
                # Check if each field is missing
                missing_attraction = 'topAttractionImageUrl' not in doc_data
                missing_restaurant = 'topRestaurantImageUrl' not in doc_data

                # Skip processing if both fields exist
                if not (missing_attraction or missing_restaurant):
                    print(f"Skipping document {doc.id} as it already has top image URLs.")
                    last_doc = doc  # Update last_doc for pagination
                    continue

                print(f"\nProcessing parent document: {doc.id}")
                parent_doc_ref = explore_collection.document(doc.id)
                last_doc = doc  # Update for pagination
                ...
                # Retrieve image URLs only if the corresponding field is missing
                top_attraction_image_url = get_top_image_url(attractions_subcol_ref) if missing_attraction else None
                top_restaurant_image_url = get_top_image_url(restaurants_subcol_ref) if missing_restaurant else None

                # Update the parent document with only the missing fields
...
#     cities_manual = [
#         {"name": "Kashmir", "id": "86838"},
#         {"name": "Gangtok", "id": "503"},
#         {"name": "Ladakh", "id": "86858"},
#         {"name": "Udiapur", "id": "91"},
#         {"name": "Andaman and Nicobar Islands", "id": "86944"},
#         {"name": "Manali", "id": "146271"},
#         {"name": "Agra", "id": "60"},
#         {"name": "Shillong", "id": "728"},
#         {"name": "Darjeeling", "id": "349"},
#         {"name": "Amristar", "id": "384"},
#         {"name": "Goa", "id": "86853"},
